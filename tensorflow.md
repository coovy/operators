...
- 然后进行tensor的数量次循环，每次循环确定一个tensor的内存分配，这是外层循环，内层就是先通过for循环遍历SizeDistPriorityInfo找到优先级最大的tensor进行分配。其实总体分配思路到这里就结束了。后面就是分配的操作细节了。  
- 如果当前SizeDistPriorityInfo的最佳距离是kNotAssigned，表示没有合适shared object供它使用，以后把shared object直接写作buffer了，则分配新buffer，否则复用best_object(和best_dist对应的那个buffer)。由于当前tensor的分配，buffer池的buffer数量可能变化，或者buffer中复用的tensor可能变化。所以需要更新其他所有SizeDistPriorityInfo的best_dist，best_object，dist这些信息。  
- tensor的数量次循环结束，分配结束
